...
>
> 本版采用**无损融合**原则：原总体方案中的数据库、API、页面 PRD、Workflow、Prompt、Eval、测试、部署、Demo 等实现级细节全部保留；同时把当前 Tool Platform、Tool Center、Action Layer、RPA、Creative Studio、Operation Automation、通用 Integration Provider Framework / MCP Runtime 等能力纳入同一个 V9 架构。
>
...
│                                                     │
│ Integration Gateway │ Provider Router │ Registry   │
│ MCP Runtime │ HTTP Adapter │ Native │ RPA          │
└─────────────────────────┬───────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────┐
│                    Providers                        │
│                                                     │
│ XYDC │ SellerSprite │ Sif │ Amazon │ ERP │ Ads    │
│ YouTube │ Search │ Image │ Video │ Other SaaS      │
└─────────────────────────┬───────────────────────────┘
                          ↓
...
EPIC-V9-10 Integration Provider Framework
EPIC-V9-11 XYDC Provider / Real Research Data
```
...
```text
    ...
    自然语言完成任务    员工直接用工具     自动运行 SOP
          ...
          └───────────────┼───────────────┘
                    ...
                    Tool Platform
                          ↓
               Integration / Execution Gateway
                          ↓
        Native / HTTP / MCP / Python / RPA / Browser
              Computer Use / Human
                          ↓
               Provider Registry / Router
                          ↓
 XYDC / SellerSprite / Sif / Amazon / ERP / Ads / Image / Video
                          ↓
                     Business Data
```
...
Tool Center
+
Integration Provider Framework / MCP Runtime
+
Action Layer
+
RPA Runtime
...
当前新增真实外部数据源时，采用独立的增量插入点：

```text
Current Increment A  Integration Provider Framework
        ↓
Current Increment B  Generic MCP Runtime
        ↓
Current Increment C  XYDC First Provider
        ↓
Current Increment D  Product Research Real Data
        ↓
Current Increment E  Additional Providers
                      SellerSprite / Sif / YouTube / ERP / ...
```

这组 Current Increment 不要求重新执行或重做已经完成的 Phase 1～6，而是在现有 Tool Platform / Integration Layer 上增量补齐 Provider 抽象。
...
# 338.31 通用 Integration Provider Framework / MCP Provider 统一规范

本节为当前生效规范，用于统一 CrossPilot 后续所有 MCP / HTTP / SaaS / 外部数据源接入方式。

## 338.31.1 定位

CrossPilot 不为每一个第三方平台开发一套独立业务架构。
...
MCP 是一种 Integration Transport，不是 Business Module。

---

## 338.31.2 Stable Tool / Capability

稳定能力继续使用 CrossPilot Tool ID 表达：
...
Agent / Workflow / Tool Center 只依赖这些稳定 Tool。
...
Workflow
→ seller_sprite.remote_tool
```

第三方 remote tool 只能存在于 Provider Binding / Adapter。

---

## 338.31.3 Provider Definition
...
MCP
        ↓
<remote tool discovered from XYDC MCP>
```

Remote Tool Name 必须通过真实 Discovery 确定，不在业务代码中猜测或散落硬编码。

---

## 338.31.5 Provider Router
...
Enabled Bindings
↓
Permission / Marketplace / Health Filter
↓
Priority
↓
Primary Provider
...
第一阶段不引入复杂 AI Provider Router。

---

## 338.31.6 MCP Runtime
...
这些仍由 Domain / Tool / Workflow / Agent 负责。

---

## 338.31.7 MCP Connection Manager
...
## 338.31.8 Secret Management

第一版可以使用 Environment Secret：

```text
XYDC_MCP_TOKEN
XYDC_MCP_ENDPOINT
SELLER_SPRITE_MCP_TOKEN
...
```

但具体业务代码不得直接读取 Secret。
...
## 338.31.9 Provider Adapter / Mapper

第三方原始 DTO 不能泄漏到 Domain。
...
Provider Mapper
↓
CrossPilot Normalized Contract
↓
...
## 338.31.10 Research 标准数据模型

Product Research 第一阶段至少统一：
...
## 338.31.11 Evidence / Data Lineage
...
禁止 Mock / Cache 静默伪装成 Live 数据。

---

## 338.31.12 Database / Trace
...
`tool_executions` 增补 Provider Metadata：
...
只有现有 Schema 无法表达动态 Provider 配置时，才新增：
...
禁止无理由再建立与 `tool_executions` 重复的 `mcp_tool_runs`。

---

## 338.31.13 Read / Action 边界
...
MCP Provider 不得绕过 Action Layer / Approval。
...
Fallback = Last Successful Snapshot / Mock
```
...
## 338.31.15 XYDC 首个 Provider

XYDC 作为 Provider Framework 的第一个真实验证对象。
...
具体 XYDC Remote Tool Name：

> **以真实 MCP Discovery 为准。**

不在本方案中写死。

---

## 338.31.16 Product Research 实际链路
...
## 338.31.18 Provider Framework 目录建议
...
以现有仓库结构为准，不为匹配文档强制搬迁旧代码。

---

## 338.31.19 Reliability
...
## 338.31.21 Definition of Done

```text
[ ] 通用 Provider Contract 已存在
[ ] MCP Runtime 无业务逻辑
[ ] Stable Tool 与 Remote Tool 解耦
[ ] XYDC DTO 未泄漏到 Domain
[ ] 至少 3 个 XYDC 真实能力跑通
[ ] Product Research 使用真实 Provider 数据
[ ] 页面可显示 LIVE / CACHED / MOCK / DEGRADED
[ ] Trace 包含 providerId / transport
[ ] Secret 已脱敏
[ ] Mock 仍可运行
[ ] Provider 故障可降级
[ ] Unit / Contract / Integration / E2E / Regression 通过
```
...
```text
A. Scan Existing Architecture
↓
B. Provider Framework Skeleton
↓
C. MCP Runtime
↓
D. XYDC Discovery
↓
E. 3～5 Capability Bindings
↓
F. Product Research Real Data
↓
G. Evidence / Source UI
↓
H. Trace / Reliability
↓
I. Regression
...
本轮禁止为了 Provider Framework 重构无关业务代码。
...
- Provider / MCP Remote Tool 直接泄漏到业务 Domain / Workflow / Agent
- Mock / Cache 被静默标记成 Live 数据
...
14. Integration Provider / MCP Runtime / Capability Binding
15. Provider Source / Data Lineage / Degraded Mode
16. E2E Business Spine
```
...
Integration Provider Framework / MCP Runtime
Provider Registry / Capability Binding
Action Layer / RPA
Creative Studio
Operation Automation
Business Intelligence
...
# 342. 本地 Coding Agent：通用 Provider Framework + XYDC 首接执行提示词

> 本节是 338.31 的执行版提示词。正式开发时，先执行 Mapping，再改代码。
...
必须遵守：
1. 禁止推倒重构现有 V8/V9。
2. 现有 Tool Platform / Registry / Executor 优先复用。
3. Stable CrossPilot Tool ID 就是内部 Capability ID。
4. Product Research / Workflow / Agent 不得直接认识 XYDC remote tool。
5. MCP 是 Transport，不是 Business Module。
6. 第三方 Raw DTO 不得泄漏到 Domain。
7. Mock 必须保留。
8. Token 不进入前端、日志、Trace 或 Git。
9. tool_executions 优先复用。
10. MCP 不得绕过 Action / Approval。

Phase 0：
只扫描代码，输出 MCP_PROVIDER_MAPPING.md。
明确：
- Tool Platform
- Tool Registry
- Tool Executor
...
负责：
connect / auth / tools-list / schema / call / timeout / bounded retry / health / error normalize / trace
...
只选 3～5 个真实能力映射到稳定 CrossPilot Tool，例如：
market.product.search
market.product.detail
market.market.overview
market.keyword.search
...
验收：
- XYDC 故障不导致 CrossPilot 整体崩溃
- 未来换 Provider 不需要修改 Product Research
- 上层不存在 XYDC remote tool name
- 第三方 DTO 不泄漏 Domain
- 至少 3 个 XYDC 真实 Tool Call 跑通
- 页面能证明 source=XYDC / mode=LIVE / capturedAt
- Trace 看得到 Provider，看不到 Token
- Mock 仍然可运行
- 无关代码不重构
