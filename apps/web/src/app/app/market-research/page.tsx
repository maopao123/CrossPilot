    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-2xl font-bold text-white tracking-tight">02 市场与选品</h1>
            <span className="text-xs bg-blue-500/20 text-blue-400 font-semibold px-2 py-0.5 rounded border border-blue-500/30">
              Milestone 3: Research & Opportunity
            </span>
          </div>
          <p className="text-sm text-gray-400 mt-1">
            Amazon US 市场宏观大盘 • 核心词大盘态势与 VOC 选品机会卡
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-surface border border-border p-4 rounded-xl">
          <span className="text-xs text-gray-400">月搜索量 (Search Volume)</span>
          ...
          <span className="text-[10px] text-emerald-400 flex items-center mt-0.5">
            <TrendingUp className="w-3 h-3 mr-0.5" /> +22.4% YoY
          </span>
        </div>

        <div className="bg-surface border border-border p-4 rounded-xl">
          <span className="text-xs text-gray-400">类目均价 (Avg Price)</span>
          <div className="text-lg font-bold text-white mt-1">
        ...
        <div className="bg-surface border border-border p-4 rounded-xl">
          <span className="text-xs text-gray-400">类目均评分 (Avg Rating)</span>
          <div className="text-lg font-bold text-white mt-1">
        ...
        <div className="bg-surface border border-border p-4 rounded-xl">
          <span className="text-xs text-gray-400">平均评论数 (Reviews)</span>
          <div className="text-lg font-bold text-white mt-1">
        ...
        <div className="bg-surface border border-border p-4 rounded-xl">
          <span className="text-xs text-gray-400">机会评分 (Opportunity)</span>
        ...
        <div className="bg-surface border border-border p-4 rounded-xl">
          <span className="text-xs text-gray-400">竞争烈度 (Competition)</span>
      ...
      <div className="bg-surface border border-border rounded-xl p-5">
        <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-3 flex items-center space-x-2">
          ...
          <span>高增长买家搜索词 (Buyer Search Queries)</span>
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {(snapshot?.trendingKeywords || []).map((k, i) => (
            <div key={i} className="bg-surface-elevated border border-border rounded-lg p-3 flex items-center justify-between">
              <div>
                <div className="text-xs font-semibold text-white">{k.keyword}</div>
                <div className="text-[11px] text-gray-400 mt-0.5">月搜: {k.volume.toLocaleString()}</div>
              </div>
        ...
        <div className="flex items-center justify-between mb-4 border-b border-border pb-3">
          <div>
            <h3 className="text-base font-bold text-white flex items-center space-x-2">
              ...
              <span>VOC 沉淀之产品定义机会卡 (Product Opportunity Brief)</span>
            </h3>
            ...
            <div key={opp.id} className="bg-surface-elevated border border-border rounded-lg p-5 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                <div className="bg-surface p-3 rounded border border-border">
                  <span className="font-semibold text-rose-400">买家核心痛点 (Problem Summary)</span>
                  <p className="text-gray-300 mt-1">{opp.problemSummary}</p>
                </div>
                <div className="bg-surface p-3 rounded border border-border">
                  <span className="font-semibold text-blue-400">目标客户群 (Target Audience)</span>
                  <p className="text-gray-300 mt-1">{opp.targetCustomer}</p>
                </div>
                <div className="bg-surface p-3 rounded border border-border">
                  <span className="font-semibold text-emerald-400">推荐产品定位 (Positioning)</span>
              ...
              <div className="text-xs text-gray-400 bg-surface/60 p-2.5 rounded border border-border/80 flex items-center justify-between">
                <div>
                  <span className="font-medium text-gray-300">依据来源 (Evidence Summary): </span>
                  <span>{opp.evidenceSummary}</span>
                </div>
                <Link
                  href="/app/listings"
                  className="text-xs bg-blue-600 hover:bg-blue-500 text-white font-semibold px-3 py-1.5 rounded flex items-center space-x-1"
                >
                  <span>直达 Listing Studio &rarr;</span>
                </Link>
              </div>
            </div>
          ))}
