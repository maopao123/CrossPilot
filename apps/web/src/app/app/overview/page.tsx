...
export default function BusinessOverviewPage() {
  ...
  const fetchOverviewData = async () => {
    try {
      ...
      console.error('Failed to load scenario overview:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetDemo = async () => {
    try {
      ...
      setResetMessage(res.message || 'Demo data reset successfully!');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-2xl font-bold text-white tracking-tight">01 经营驾驶舱</h1>
            <span className="text-xs bg-emerald-500/20 text-emerald-400 font-semibold px-2 py-0.5 rounded border border-emerald-500/30">
              90-Day Full Lifecycle Active
            </span>
          </div>
          <p className="text-sm text-gray-400 mt-1">
            SKU 360 运营平台 • POLEGAS Natural Marble Toothbrush Holder (Amazon US)
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            disabled={resetting}
            className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-semibold px-4 py-2 rounded-lg transition shadow-md"
          >
            <RotateCcw className={`w-3.5 h-3.5 ${resetting ? 'animate-spin' : ''}`} />
            <span>{resetting ? 'Resetting Demo...' : '1-Click Demo Reset (重置基准数据)'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        <div className="bg-surface border border-border rounded-xl p-4">
          <span className="text-xs text-gray-400">90天总销售额 (Revenue)</span>
          <div className="text-xl font-bold text-white mt-1">
            ${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
          <span className="text-[11px] text-emerald-400 flex items-center mt-0.5">
            Avg Daily: ${(totalRevenue / 90).toFixed(0)}/day
          </span>
        </div>

        <div className="bg-surface border border-border rounded-xl p-4">
          <span className="text-xs text-gray-400">90天净利润 (Net Profit)</span>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <span className="text-xs text-gray-400">总订单件数 (Units Sold)</span>
          <div className="text-xl font-bold text-white mt-1">
            {totalOrders.toLocaleString()} pcs
          </div>
          <span className="text-[11px] text-gray-400 mt-0.5">
            3个变体协同出货
          </span>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <span className="text-xs text-gray-400">广告总支出 (Ads Spend)</span>
          <div className="text-xl font-bold text-white mt-1">
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <span className="text-xs text-gray-400">Week 11 利润异动</span>
            ...
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                <span className="text-xs font-bold text-blue-400 bg-blue-950/60 px-2 py-0.5 rounded border border-blue-800">
                  {selectedEvent.code} (Day {selectedEvent.day})
                </span>
                <span className="text-xs font-semibold text-gray-300">{selectedEvent.skuCode || 'ALL SKUs'}</span>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 flex-shrink-0">
              ...
              {(selectedEvent.code === 'E04' || selectedEvent.code === 'E05') && (
                <Link href="/app/inventory" className="text-xs bg-amber-600 hover:bg-amber-500 text-white font-semibold px-3 py-2 rounded-lg flex items-center justify-center space-x-1">
                  <span>执行补货决策 (12 Days Cover)</span> <ChevronRight className="w-3.5 h-3.5" />
                </Link>
              )}
              ...
              {selectedEvent.code === 'E08' && (
                <Link href="/app/listings" className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white font-semibold px-3 py-2 rounded-lg flex items-center justify-center space-x-1">
                  <span>Listing Studio 合规锁版</span> <ChevronRight className="w-3.5 h-3.5" />
                </Link>
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded">Steady Growth</span>
          </div>
          <div className="text-sm font-bold text-white mt-2">MTH-WHITE-001</div>
          <p className="text-xs text-gray-400 mt-1">
            日均销量 14~18 pcs • 售价 $29.99 • 毛利率 23.4% • 退货率 2.1% 稳健
          </p>
          <div className="mt-3 pt-3 border-t border-border flex items-center justify-between text-xs">
            <span className="text-gray-400">库存水位: 420 pcs</span>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded">Surge & Stockout</span>
          </div>
          <div className="text-sm font-bold text-white mt-2">MTH-GREEN-001</div>
          <p className="text-xs text-gray-400 mt-1">
            社媒带货激增至 35 pcs/day • 触发 12天断货预警 (E04) • 补货 500 pcs 入库
          </p>
          <div className="mt-3 pt-3 border-t border-border flex items-center justify-between text-xs">
            <span className="text-amber-400 font-semibold">曾触及 12 Days Cover</span>
            <Link href="/app/inventory" className="text-blue-400 hover:underline">补货中心 &rarr;</Link>
          </div>
        </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded">VOC Improved</span>
          </div>
